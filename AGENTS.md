...
- Do not push `main`, the feature branch or `app_common` without authorization. After the merge is committed, remove the worktree (`git worktree remove --force ../SuperBirdTools-<feature-branch>`; `--force` is required because the worktree contains the `app_common` submodule, so confirm the branch is committed and merged first) and delete the merged branch unless the user wants it kept; report any worktree left behind and why.
