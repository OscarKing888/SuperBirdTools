...
- HIF/HEIC/HEIF and RAW full-preview decoding belongs in the asynchronous full-preview worker. Keep HIF/HEIF fallback through Pillow/`pillow-heif` and RAW fallback through `rawpy`; never move those full decodes into the `load_full=False` fast keyboard path.
...
- Directory selection in SuperViewer may recursively discover large image sets. Keep scanning/listing/thumbnail preparation asynchronous and progress-reporting; do not apply tens of thousands of rows or metadata entries in one blocking GUI-thread pass.
- Switching directories should prioritize thumbnail generation for the newly selected directory without canceling already queued thumbnail work for previous directories.
...
- SuperViewer JSON sidecars use `<image filename>.superviewer.json` beside the source image. Metadata writes may prefer this JSON sidecar while XMP compatibility remains readable.
- Deleting files through `move_to_trash` should default to `.superpicky/deleted/`, preserving the original path relative to the `.superpicky` root. Related sidecar files such as `.xmp` and `.superviewer.json` must move with the image into the matching deleted path.
- Copy/paste and duplicate-style file operations must carry both supported sidecar families with the image: sibling `.xmp` and `<image filename>.superviewer.json`.
