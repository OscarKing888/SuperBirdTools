...
- Create the worktree as a sibling directory of the checkout root, named after the branch, and branch from the current `main`:
  ...
  then `git submodule update --init app_common` inside the new worktree so `app_common` is populated. If that fails with "not our ref", the `main` gitlink points at an `app_common` commit that exists only in the main checkout; run `git -C app_common fetch <repo>/app_common main` in the worktree and repeat the submodule update instead of pushing or resetting the gitlink. Existing worktrees (for example `../SuperBirdTools-img_mgr`) belong to other rounds; never edit, reset or remove them unless the user asks.
...
- `app_common` is a git submodule with its own repository (`https://github.com/OscarKing888/SuperAppCommonLib.git`), not a directory of this repo. Any change under `app_common/` must be committed inside the submodule with `git -C app_common add <paths>` and `git -C app_common commit` (on its `main`); the superproject cannot contain those file changes and `git add app_common` only records the new gitlink commit id. Uncommitted submodule edits show up as `M app_common` ("modified content") in the superproject and are lost if only the superproject is committed, so check `git -C app_common status --short` before every superproject commit.
- Commit on the feature branch, following that order: `app_common` first (on its `main`), then the superproject gitlink plus integration changes in one superproject commit so the two stay reviewable together. Stage explicit paths only; the `main` checkout may carry unrelated user modifications (runtime config, `run.sh`, gitlink) that must not be folded into the feature branch.
...
- Do not push `main`, the feature branch or `app_common` without authorization.
- Feature branches and their worktrees are temporary: once the merge into `main` is committed, always remove the worktree (`git worktree remove --force ../SuperBirdTools-<feature-branch>`; `--force` is required because the worktree contains the `app_common` submodule, so confirm the branch is committed and merged first) and delete the branch (`git branch -d <feature-branch>`) in the same round. Never keep a merged feature branch or worktree "just in case"; `git worktree list` and `git branch --list` should show only `main` plus rounds that are still unmerged. If cleanup fails, report it and why instead of leaving it silently.
...
- Normal single-image selection:
  ...
  - For HEIF/HIF above the HEIF threshold, first reuse the exact selected-tier cache. If no cached quick image exists, show a loading placeholder and decode the full preview in the owned worker; Pillow HEVC thumbnailing would otherwise synchronously decode the entire image only to shrink it. Do not decode it twice to fabricate an intermediate quick frame.
  - RAW images use a two-stage display: the GUI thread only shows the exact selected-tier cached preview (or a loading placeholder when uncached) and never extracts or decodes RAW data; the owned full-preview worker then replaces it with the high-resolution embedded RAW preview JPEG. `source_pixmap_for_path()` and `full_preview_ready` only become true once that embedded preview arrives.
  - RAW embedded-preview extraction (`thumb_stream.get_raw_preview_jpeg`) prefers the in-process LibRaw (`rawpy.extract_thumb`) camera preview when its long edge is at least `RAW_INPROCESS_PREVIEW_MIN_LONG_EDGE` (1600); otherwise it queries ExifTool `JpgFromRaw` → `PreviewImage` → `ThumbnailImage` and keeps the larger result. Tiny piexif EXIF thumbnails such as 160x120 remain last-resort fallbacks. Do not force a full RAW demosaic for ordinary preview switching.
...
- Burst group frames: list and thumbnail modes draw a semi-transparent background band behind each burst group (same directory + same `burst_id`, at least 2 visible members), alternating two colors by group order. Grouping lives in the shared `_BurstGroupMixin` of `FileTableModel` / `ThumbnailListModel` (`_compute_burst_group_rows()`), list mode exposes it through `BackgroundRole`, thumbnail mode through `_MetaBurstGroupRole` painted by `ThumbnailItemDelegate`. `FileListPanel._sync_burst_group_display()` turns frames off whenever `_has_any_filter()` is true (including SuperViewer tag filters); keep `app_common/tests/test_file_browser_burst_groups.py` passing when touching this.
...
- SuperViewer thumbnail loading, persistent thumbnail generation, and metadata reading share one bounded `BrowserWorkPool` per browser across directory changes. Prioritize visible thumbnails, then prefetch, then persistent generation; reserve at least 2 workers for metadata so it cannot starve. Default metadata reservation is roughly `CPU/4` capped at 8 (minimum 2); default thumbnail capacity uses the remaining CPU threads, e.g. 32 total gives 8 metadata and 24 thumbnail workers. All threads must execute the same `WorkerAction.execute()` interface; metadata, viewport thumbnails and persistent thumbnails are concrete actions, while `BrowserWorkPolicy` owns priorities and concurrency quotas. Reservations are quotas, never fixed thread identities. Metadata may use the entire pool after thumbnail work finishes, and thumbnails may use the entire pool after metadata finishes. Keep total thread count bounded; reschedule borrowed capacity at action boundaries when priority work returns. Coordinator demand leases must cover submission gaps and release on idle/completion/cancellation so redistribution wakes workers without waiting for GUI progress callbacks. Refill jobs on individual completion rather than batch barriers, and use small metadata chunks. Each pool thread owns a reusable read-only ExifTool session with timeout/cancellation, avoiding the global session lock. Keep progress text showing actual active thread counts and preserve worker ownership until real completion. Shared FileListPanel defaults this pool off so SuperBirdStamp retains its existing behavior.
